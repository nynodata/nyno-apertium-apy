...
class FlushingPipeline(Pipeline):
    ...
    def __del__(self):
        ...
        logging.info('shutting down FlushingPipeline that was used %d times', self.use_count)  # HPLS
...
def make_pipeline(modes_parsed, timeout):
    logging.info('make_pipeline:do_flush%d, :%s', modes_parsed.do_flush, modes_parsed)  # HPLS show cmd
...
def start_pipeline(commands):
    logging.info('commands: %s', commands)  # HPLS
...
def parse_mode_file(mode_path):
    logging.info('parse mode file path:%s', mode_path)  # HPLS show cmd
        ...
        if 'ca-oc@aran' in mode_str:
            ...
            for cmd in mode_str.strip().split('|'):
                ...
                commands.append([c.strip("'")
                                 for c in cmd.split()])
        logging.info('doflush:%d, cmd:%s', do_flush, commands)  # HPLS show cmd
...
def split_for_translation(to_translate, n_users):
    ...
    while last < len(to_translate) and rounds < 10:
        ...
        # logging.getLogger().setLevel(logging.DEBUG)  # comment removed HPLS
...
async def translate_simple(to_translate, commands, prefs=''):
    logging.info('translate_simple: %s', commands)  # HPLS
...
def start_pipeline_from_modefile(mode_file, fmt, stilmal, unknown_marks=False):
    logging.info('start_pipeline_from_modefile: %s', mode_file)  # HPLS
    ...
    if unknown_marks:
        ...
        cmd = ['apertium', '-f', fmt, '-D', 'd-' + stilmal, '-u', '-d', modes_dir, mode]  # HPLS
    return start_pipeline([cmd])


async def translate_modefile_bytes(to_translate_bytes, fmt, mode_file, stilmal, unknown_marks=False):
    logging.info('translate_modefile_bytes: %s', mode_file)  # HPLS
    proc_in, proc_out = start_pipeline_from_modefile(mode_file, fmt, stilmal, unknown_marks)
    assert proc_in == proc_out
...
@gen.coroutine
def translate_html_mark_headings(to_translate, mode_file, stilmal, unknown_marks=False):
    translated = yield translate_modefile_bytes(bytes(to_translate, 'utf-8'), 'html', mode_file, stilmal, unknown_marks)
