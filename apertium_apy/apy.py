...
def parse_args(cli_args=sys.argv[1:]):
    ...
    parser.add_argument('-p', '--port', help='port to run server on (default = 3780)', type=int, default=3780)  # HPLS default port 3780
...
def setup_application(args):
    ...
    handlers = [
        # TODO: HPLS remove the handlers we dont use. -> Ok.
        #       Pull a branch and make it public.
        (r'/', RootHandler),
        # (r'/list', ListHandler),
        # (r'/listPairs', ListHandler),
        (r'/stats', StatsHandler),
        # (r'/pairprefs', PairPrefsHandler),
        # (r'/translate', TranslateHandler), # HPLS renamed to omsettTekst
        (r'/translateText', TranslateHandler),
        # (r'/translateChain', TranslateChainHandler),
        # (r'/translateDoc', TranslateDocHandler), # HPLS renamed to omsettWord
        (r'/translateWord', TranslateDocHandler),
        # (r'/translatePage', TranslateWebpageHandler), # HPLS renamed to omsettHtml
        (r'/translateHtml', TranslateWebpageHandler),
...
def setup_logging(args):
    if args.daemon:
        ...
        # logging.getLogger().setLevel(logging.INFO) HPLS
