...
class TranslateWebpageHandler(TranslateHandler):
    ...
    def readJsFile(self, filename):
        # <body onload="javascript:SomeFunction()">
        text_file = open(os.path.dirname(os.path.realpath(__file__)) + filename, "r")
        data = text_file.read()
        text_file.close()
        return data
    ...
    def unescape(self, page):
        # HPLS
        # text_file = open("/home/apertium/hp_dev/tmp/in.html", "w")
        ...
        # Windows-based web pages, see
        # http://stackoverflow.com/a/1398921/69663 :
        # sp = self.get_argument('stilmal')  # HPLS
        page = page.replace('&#150;', '&#8211;')
        demo = self.get_argument('demo', 'no')  # HPLS
        if demo == 'yes':
            js = self.readJsFile('/link_listener.html')  # HPLS
            page = page.replace('</head>', js + '\n</head>')  # HPLS
        # page = '<!--<apertium-notrans>TP=HTML,SP=' + sp + '</apertium-notrans>-->' + page  # HPLS
        # page = page.replace('"', "'")  # HPLS Hack: html.escape esapes dobble quote
        # Unescape all other entities the regular way:
        return html.unescape(page)
    ...
    @ gen.coroutine
    def get(self):
        pair = self.get_pair_or_error(self.get_argument('langpair', 'nob_h|nno'),
        ...
        stilmal = self.get_argument('stilmal').replace(' ', '_')  # HPLS
        if not url.startswith('http'):
            url = 'http://' + url
        got304 = False
        cached = self.get_cached(pair, url)
        logging.info('request webpage url:%s', url)  # HPLS show cmd
        ...
        if got304 and cached is not None:
            ...
            translated = yield translation.translate_html_mark_headings(to_translate, mode_path, stilmal)
        ...
        # HPLS
        # text_file = open("/home/apertium/hp_dev/tmp/out.html", "w")
        # n = text_file.write(self.maybe_strip_marks(self.mark_unknown, pair, translated))
        # text_file.close()
        # HPLS
        ...
        if got304 and retranslate is not None:
            ...
            translated = yield translation.translate_html_mark_headings(retranslate, mode_path, stilmal)
