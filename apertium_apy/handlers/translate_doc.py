...
async def translate_doc(file_to_translate, fmt, mode_file, stilmal, unknown_marks=False):
    logging.info('translate_doc: %s, Stilmal:%s', mode_file, stilmal)  # HPLS
    ...
    if unknown_marks:
        ...
        cmd = ['apertium', '-f', fmt, '-D', 'd-' + stilmal, '-u', '-d', modes_dir, mode]
    logging.info('apertium cmd :%s', cmd)  # HPLS show cmd
...
class TranslateDocHandler(TranslateHandler):
    ...
    def get_mime_type(cls, f):
        ...
        logging.info('mime_command :%s', mime_command)  # HPLS show cmd
        logging.info('mime_type :%s', mime_type)  # HPLS show cmd
    ...
    @gen.coroutine
    def get(self):
        pair = self.get_pair_or_error(self.get_argument('langpair', 'nob_d|nno'), -1)
        ...
        if pair is not None:
            ...
            with tempfile.NamedTemporaryFile() as temp_file:
                # temp_file.write("<apertium-notrans>TP=TEXT,SP=Konservativ</apertium-notrans>")
                temp_file.write(body)
                temp_file.seek(0)
                # copyfile(temp_file.name, temp_file.name + "_yoo")
                ...
                with (yield self.doc_pipe_sem.acquire()):
                    t = yield translate_doc(temp_file,
                                            ...
                                            self.pairs['%s-%s' % pair],
                                            stilmal, self.mark_unknown)  # HPLS
                self.write(t)
                self.finish()
