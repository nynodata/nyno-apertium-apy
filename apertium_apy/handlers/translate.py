...
class TranslationInfo:
    def __init__(self, handler):
        self.langpair = handler.get_argument('langpair', default='nob_t|nno')  # set default
        self.stilmal = handler.get_argument('stilmal', default='Moderat nynorsk')
        ...
        logging.info('langpar:%s, stilmal:%s, key:%s, ip:%s, referer:%s', self.langpair, self.stilmal, self.key, self.ip, self.referer)
...
class TranslateHandler(BaseHandler):
    ...
    def mark_unknown(self):
        return self.get_argument('markUnknown', default='no').lower() in ['yes', 'true', '1']  # HPLS yes-> no
    ...
    def translate_and_respond(self, pair, pipeline, to_translate, mark_unknown, nosplit=False, deformat=True, reformat=True, prefs=''):
        ...
        try:
            ...
            if translated[:6] == '10404:':
                logging.warning('Translation error %s', translated)
                self.send_error(503, explanation='Error execute api - service call. Probertly wrong stilmal')
            else:
                self.send_response({
                    'responseData': {
    ...
    @gen.coroutine
    def get(self):
        pair = self.get_pair_or_error(self.get_argument('langpair', default='nob_t|nno'),  # HPLS Set sefault
        ...
        if pair is not None:
            ...
            yield self.translate_and_respond(pair,
                                             pipeline,
                                             "<apertium-notrans>TP=TEXT,SP=" +
                                             self.get_argument('stilmal') +
                                             "</apertium-notrans>" +
                                             self.get_argument('q') + "\n*",  # HPLS add \n*
                                             self.get_argument('markUnknown', default='no'),  # HPLS yes to no
                                             nosplit=False,
                                             deformat=deformat,
                                             reformat=reformat,
